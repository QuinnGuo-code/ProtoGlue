"""Run ProtoGlue on Human Lymph Node dataset (RNA + ADT)."""

from pathlib import Path
import protoglue as pg

result = pg.run_pipeline(
    data_dir=Path("data/human_lymph_node"),
    output_dir=Path("results/human_lymph_node"),
    config={"K_GRID": list(range(5, 9))},
    device="cuda",
    seed=2022,
    auto_k=True,
    verbose=True,
)

print(f"Clusters: {result['metrics']['n_clusters']}")
print(f"ARI: {result['metrics'].get('ARI', 'N/A')}")
print(f"NMI: {result['metrics'].get('NMI', 'N/A')}")
print(f"Spatial purity: {result['metrics']['spatial_edge_purity']:.4f}")
